...
export default function App() {
  ...
  const [musicPlaying, setMusicPlaying] = useState(sound.musicPlaying);
  const configured = isFirebaseConfigured();
  const onHome = !localConfig && !identity;
  useEffect(() => {
    if (!onHome) { sound.stopMusic(); return; }
    setMusicEnabled(sound.enabled && sound.musicEnabled);
    setMusicPlaying(sound.musicPlaying);
    const startMusic = (): void => {
      if (sound.enabled && sound.musicEnabled) void sound.startMusic().then(() => setMusicPlaying(sound.musicPlaying)).catch(() => setMusicPlaying(false));
    };
  ...
  return (
    <main className="home-screen">
      ...
      <footer className="home-footer"><span>Motor determinista · IA sin acceso a roles secretos · Firebase RTDB</span><VersionChangelog /></footer>
    </main>
  );
}
